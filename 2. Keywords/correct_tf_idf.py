...
def get_betweenness_centrality(filename, all_texts, lower, sw):
    G = create_graph(filename, all_texts, lower, sw)
    bc = nx.betweenness_centrality(G)
    return bc


def draw_graph(filename, graphname, all_texts, lower, sw):
    G = create_graph(filename, all_texts, lower, sw)
    bc = get_betweenness_centrality(filename, all_texts, lower, sw)
...
def get_top_betweenness_centrality(filename, all_texts, lower, sw):
    bc = get_betweenness_centrality(filename, all_texts, lower, sw)
    candidates = create_coocurence_matrix(filename, all_texts, lower, sw)[0]
...
def generate_metrics(filename, all_texts, lower, sw):
    top_betweenness_centrality = get_top_betweenness_centrality(filename, all_texts, lower, sw)
    true = open(filename + '.key', 'r', encoding='utf-8').readlines()
    true = [line.split('\n')[0] for line in true]
    print(get_top_betweenness_centrality(filename, all_texts, lower, sw), '\n')
    print(true, '\n')
...
results = open('metrics_test.csv', 'w', encoding='utf-8')
